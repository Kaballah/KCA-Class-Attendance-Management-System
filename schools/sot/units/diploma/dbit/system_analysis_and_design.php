    ...
    <form action="./database/insert.php" method="post">
        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
            <div class="alert alert-success">
                <h2 style="text-align:center; font-family:cursive;">System Analysis and Design</h2>
            </div>
            <thead>
                <tr>
            ...
            <tbody>
            <?php 
                $query = mysqli_query($dbhandle, "SELECT * FROM system_analysis_and_design_dbit_2")or die(mysql_error());
                while($row = mysqli_fetch_assoc($query)){
                    $id = $row['regNumber'];
                    $email = $row['name'];

                    $checklistSQL = mysqli_query($dbhandle, "SELECT * FROM system_analysis_and_design_dbit_2")or die(mysql_error());
