...
{ 
    ...
    $email = $row['name']; 
    $checklistSQL = $connect->query("SELECT * FROM cbit_students WHERE RegNumber = '$member_id' "); 
    $answer = $_POST['answer'];
    ...
    <form action="./database/insert.php" method="post">
        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
            <div class="alert alert-success">
                <h2 style="text-align:center; font-family:cursive;">Business and Enterprise Management</h2>
            </div>
            <thead>
                <tr>
            ...
            <tbody>
            <?php 
                $query = mysqli_query($dbhandle, "SELECT * FROM business_and_enterprise_management_cbit")or die(mysql_error());
                while($row = mysqli_fetch_assoc($query)){
                    $id = $row['regNumber'];
                    $email = $row['name'];

                    $checklistSQL = mysqli_query($dbhandle, "SELECT * FROM business_and_enterprise_management_cbit")or die(mysql_error());
