...
<body>
    ...
    <form action="./php_connects/insert.php" method="post">
        <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
            ...
            <tbody>
            <?php 
                $query = mysqli_query($dbhandle, "SELECT * FROM custom_relations_cbit")or die(mysql_error());
                while($row = mysqli_fetch_assoc($query)){
                    $id = $row['regNumber'];
                    $email = $row['name'];

                    $checklistSQL = mysqli_query($dbhandle, "SELECT * FROM custom_relations_cbit")or die(mysql_error());
