<?php 
  ...
  $qurty=mysqli_query($conn,"SELECT * from business_information_management_cbit  WHERE dateTimeTaken='$dateTaken'");
  ...
  if($count == 0) { //if Record does not exsit, insert the new record
    ...
    $qus=mysqli_query($conn,"SELECT * FROM students_table WHERE unit3 = 'Business Information Management' ");
    while ($ros = $qus->fetch_assoc()) {
      $qquery=mysqli_query($conn,"INSERT INTO business_information_management_cbit (regNumber, firstName, lastName, attendance, remarks, dateTimeTaken) 
  ...
  if(isset($_POST['save'])) {
    ...
    $qurty=mysqli_query($conn,"SELECT * FROM business_information_management_cbit  WHERE dateTimeTaken='$dateTaken' AND attendance = '1'");
    $count = mysqli_num_rows($qurty);
    ...
    if($count > 0) {
      ...
      for($i = 0; $i < $N; $i++) {
        ...
        if(isset($check[$i])) {
          $qquery=mysqli_query($conn,"UPDATE business_information_management_cbit SET attendance='1' WHERE regNumber = '$check[$i]' AND dateTimeTaken='$dateTaken'");
                    ...
                    <div class="card mb-4">
                      ...
                      <div class="table-responsive p-3">
                        ...
                        <table class="table align-items-center table-flush table-hover">
                          ...
                          <tbody>
                            <?php
                              $query = "SELECT * FROM students_table WHERE unit3 = 'Business Information Management'";
                              $rs = $conn->query($query);
                              $num = $rs->num_rows;
                              $sn=0;
                              $status="";
